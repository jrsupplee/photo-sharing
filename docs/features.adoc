...
=== Creating Events
* Set event name, URL slug, event date, and close-uploads date
* Define albums upfront or add them later
* Option to require uploaders to enter their name

=== Managing an Event (General Tab)
* Edit name, event date, close-uploads date, and require-name setting
...
* #Set an *available from* date on an album: guests cannot upload before that date; admins and event managers are unaffected#
* #*Hide* an album from guests: hidden albums are invisible in the guest gallery and upload form; admins and event managers see them with a hidden indicator icon#
