    ...
    <div className="min-h-screen bg-cream">
      <header className="border-b border-stone-100 bg-white/50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="font-cormorant text-2xl text-stone-700">
              {admin ? 'Admin Dashboard' : 'Event Manager'}
            </h1>
            <p className="text-stone-400 text-xs font-light">{session.user.name}</p>
          </div>
          <div className="flex items-center gap-3">
            {admin && (
              <Link
                href="/admin/users"
                className="text-stone-500 hover:text-stone-700 transition-colors text-sm tracking-wider"
              >
                Users
              </Link>
            )}
        ...
        {events.length === 0 ? (
          <div className="text-center py-20 text-stone-400">
            ...
            <p className="font-light italic">
              {admin ? 'No events yet. Create your first wedding event.' : 'No events assigned to you yet.'}
            </p>
          </div>
        ) : (
