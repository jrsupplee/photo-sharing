...
export function initEventPermissionsTable(db: Database.Database): void {
  db.exec(`
...
export const eventPermissionRepo = {
  ...
  hasPermission(userId: number | string, eventId: number | string): boolean {
    return !!getDb().prepare('SELECT 1 FROM event_permissions WHERE user_id = ? AND event_id = ?').get(userId, eventId);
  },

  /** Replace all event permissions for a user */
  setForUser(userId: number | string, eventIds: number[]): void {
    const db = getDb();
    const insert = db.prepare('INSERT OR IGNORE INTO event_permissions (user_id, event_id) VALUES (?, ?)');
    db.transaction(() => {
      db.prepare('DELETE FROM event_permissions WHERE user_id = ?').run(userId);
      for (const eventId of eventIds) insert.run(userId, eventId);
    })();
  },
  ...
  setForEvent(eventId: number | string, userIds: number[]): void {
    ...
    const insert = db.prepare('INSERT OR IGNORE INTO event_permissions (user_id, event_id) VALUES (?, ?)');
    db.transaction(() => {
      db.prepare('DELETE FROM event_permissions WHERE event_id = ?').run(eventId);
      for (const userId of userIds) insert.run(userId, eventId);
    })();
  },
};
