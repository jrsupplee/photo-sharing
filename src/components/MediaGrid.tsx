    ...
    <>
                ...
                {isVideo ? (
                  <div className="relative w-full bg-stone-900 aspect-video flex items-center justify-center">
                    {item.thumbnail_key ? (
                      ...
                      <img src={`/api/files/${item.thumbnail_key}`} alt={item.caption || item.original_name} className="w-full h-full object-cover" />
                    ) : (
      ...
      {videoOpen && currentMedia?.mime_type?.startsWith('video/') && (
        <VideoModal
          ...
          sessionId={sessionId}
          isAdmin={isAdmin}
          isLiked={likedIds.has(currentMedia.id)}
          likeCount={likeCounts[currentMedia.id] || 0}
          commentCount={commentCounts[currentMedia.id] || 0}
          onLike={(count, liked) => {
            setLikeCounts(prev => ({ ...prev, [currentMedia.id]: count }));
            setLikedIds(prev => {
              const next = new Set(prev);
              liked ? next.add(currentMedia.id) : next.delete(currentMedia.id);
              return next;
            });
          }}
          onCommentAdded={(mediaId) => {
            setCommentCounts(prev => ({ ...prev, [mediaId]: (prev[mediaId] || 0) + 1 }));
          }}
          onClose={() => { setVideoOpen(false); setLightboxOpen(true); }}
          onDeleted={(id) => {
            setVideoOpen(false);
            setLightboxOpen(false);
